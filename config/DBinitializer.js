...
const initializer = (reset = false) => {
  ...
  let sql_query =
    "CREATE TABLE Articles (uuid char(36) NULL,title varchar(150) NOT NULL, description varchar(1000) NOT NULL, author varchar(100) NOT NULL );";
  sql.query(sql_query, (err, res) => {
    if (err) {
  ...
  let prom = new Promise((resolve, reject) => {
    let sql_query =
      "create table Authors( email varchar(100) primary key,name varchar(255)not null,password varchar(255) not null )";
    sql.query(sql_query, (err, res) => {
      if (err) {
  ...
  prom
    ...
    .then(() => {
      let sql_query =
        "CREATE TRIGGER before_insert_Articles BEFORE INSERT ON Articles FOR EACH ROW BEGIN IF new.uuid IS NULL THEN SET new.uuid = uuid(); END IF; END ;;";
